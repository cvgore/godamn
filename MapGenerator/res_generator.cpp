...
namespace Godamn
{
	...
	res_generator::res_generator(game_map* map, int min_node_count, int max_node_count,
		...
		n_original_max_nodes((los(min_node_count, max_node_count)) / 2 * 2),
		n_original_max_tiles(((min_node_count* los(min_tiles_in_node, max_tiles_in_node)) + 1) / 2 * 2),
	...
	{
		return IPoint(los(0, n_map->getLenght()), los(0, n_map->getWidth()));
	}
	...
	{
		int max_tiles_in_this_node = (n_avg_tiles_in_node + los(-1, 1));
		...
		for (int i = 0; i < max_tiles_in_this_node; i++)
		{
			...
			int switch_count = 0;
		randomize:
			int movement_randomizer = los(0, 4);
